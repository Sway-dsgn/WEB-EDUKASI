...
const MAX_TIME = 1200;
const PROGRESS_KEY = 'eduvix_quiz_progress_01';
...
function saveResult(score, correct) {
  ...
  if (window.EduvixAPI) {
    EduvixAPI.simpanHasilQuiz('quiz_01', score, correct, quizData.length - correct, quizData.length)
      .then(res => {
        ...
        EduvixAPI.selesaikanMateri('materi_01', 0).catch(() => {});
      })
      .catch(err => {
...
function loadQuizProgress() {
    try {
        ...
        if (totalXP > 0) dom.xpBadge.textContent = `+${totalXP} XP`;
    } catch (e) {
